...
echo "Season Ticket Manager - QNAP Setup (No Sudo Required)"
echo "===================================================="

# Configuration
PROJECT_PATH="/share/Container/projects/SeasonTicketTracker"
...
if [ "$EUID" -eq 0 ]; then
    print_warning "Running as root. Consider running as regular user for better security."
fi

# Try to create project directory without sudo first
print_step "Setting up project directory..."
if [ ! -d "$PROJECT_PATH" ]; then
    if mkdir -p "$PROJECT_PATH" 2>/dev/null; then
        print_success "Created directory: $PROJECT_PATH"
    else
        print_error "Cannot create $PROJECT_PATH without sudo privileges"
        echo "Please run one of these commands first:"
        echo "  sudo mkdir -p $PROJECT_PATH"
        echo "  sudo chown \$USER:users /share/Container/projects"
...
cd "$PROJECT_PATH" || {
    print_error "Cannot access $PROJECT_PATH"
    exit 1
}

# Check LXD access without sudo
print_step "Checking LXD access..."
if ! lxc list >/dev/null 2>&1; then
    print_error "Cannot access LXD without sudo"
    echo ""
    echo "Quick fix options:"
    echo "1. Add your user to lxd group:"
    echo "   sudo usermod -a -G lxd \$USER"
    echo "   newgrp lxd"
    echo ""
    echo "2. Or run with sudo but use the -S option:"
    echo "   echo 'your-password' | sudo -S ./setup-custom-path.sh"
    echo ""
    echo "3. Or use the Container Station GUI method (see CONTAINER-STATION-LXD-SETUP.md)"
    exit 1
fi

print_success "LXD access confirmed"
...
<div class="container"><div id="auth-section" class="card"><div class="header"><h1>Season Ticket Manager</h1><p>QNAP LXD Container - No Sudo Required</p></div>
...
<div class="welcome"><h3>Passwordless Deployment Successful</h3><p>Running on QNAP LXD without sudo requirements</p></div>
...
app.listen(port, "0.0.0.0", () => {
  ...
  console.log(`Platform: QNAP LXD Container (No Sudo)`);
...
echo ""
print_success "Passwordless Deployment Complete!"
echo "================================="
...
echo ""
echo "Management Commands (no sudo needed):"
...
echo ""
print_success "Season Ticket Manager deployed without sudo requirements!"