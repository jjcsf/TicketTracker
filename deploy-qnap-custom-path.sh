...
echo "============================================"
echo "Custom Path: /share/Container/projects/SeasonTicketTracker"
...
if ! command -v lxc &> /dev/null; then
    print_error "LXD not found. Please install LXD on your QNAP system first."
    exit 1
fi

print_step "Preparing LXD container deployment..."
...
# Add port forwarding
print_step "Setting up port forwarding on port $APP_PORT..."
...
lxc exec "$CONTAINER_NAME" -- bash -c "
    ...
    apt install -y -qq nodejs build-essential curl
"
...
# Create application server with embedded frontend
...
<head>
  ...
  <title>Season Ticket Manager - QNAP Container</title>
  <style>
...
# Check port binding
if lxc exec "$CONTAINER_NAME" -- netstat -tulpn | grep -q ":$APP_PORT"; then
    print_success "Port $APP_PORT is properly bound"
else
    print_error "Port $APP_PORT is not bound"
fi

# Get QNAP IP and test endpoints
...
echo "2. Create a test account using the registration form"
echo "3. Sign in with your new credentials"
echo "4. Verify the dashboard loads correctly"
echo ""
echo "MANAGEMENT COMMANDS:"
echo "  Container status:  lxc list"
...
echo "  Restart service:   lxc exec $CONTAINER_NAME -- systemctl restart season-ticket-manager"
...
echo ""
print_success "Season Ticket Manager is now ready for use!"
