...
echo "Season Ticket Manager - Final QNAP LXD Deployment"
echo "================================================="
...
# Deploy the container if it doesn't exist
if ! lxc list | grep -q "$CONTAINER_NAME"; then
    print_step "Creating and deploying new container..."
...
EOF
    ...
    print_info "Container already exists. Using existing container."
fi

# Deploy/update application code
...
if lxc exec "$CONTAINER_NAME" -- netstat -tulpn | grep -q ":$APP_PORT"; then
    print_success "Port $APP_PORT is bound"
else
    print_error "Port $APP_PORT is not bound"
fi

# Get QNAP IP and test
...
echo ""
print_success "Season Ticket Manager deployment completed successfully!"
