...
# One-Command QNAP LXD Deployment for Season Ticket Manager
set -e

CONTAINER_NAME="season-ticket-manager"
APP_PORT="5050"

echo "Season Ticket Manager - QNAP LXD One-Command Deploy"
echo "=================================================="

# Check if running as root
if [ "$EUID" -ne 0 ]; then
    echo "Please run as root: sudo bash <(curl -sSL url-to-this-script)"
    exit 1
fi
...
# Create the application directly in the container
echo "Creating Season Ticket Manager application..."
lxc exec "$CONTAINER_NAME" -- su -c 'cat > /opt/app/server.js << '"'"'EOF'"'"'
#!/usr/bin/env node
