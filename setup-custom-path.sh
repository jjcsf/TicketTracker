...
echo "Season Ticket Manager - QNAP Setup with Custom Path"
echo "===================================================="
...
if [ "$EUID" -ne 0 ]; then
    print_error "Please run as root: sudo ./setup-custom-path.sh"
    exit 1
fi
...
if [[ "$CURRENT_DIR" != "$PROJECT_PATH" ]]; then
    ...
    # Copy current script and files if needed
    if [[ -f "quick-setup.sh" ]]; then
        cp *.sh "$PROJECT_PATH/" 2>/dev/null || true
        cp *.md "$PROJECT_PATH/" 2>/dev/null || true
        chmod +x "$PROJECT_PATH"/*.sh 2>/dev/null || true
        print_success "Files copied to project directory"
    fi
...
echo ""
print_success "Season Ticket Manager is ready at your custom project path!"